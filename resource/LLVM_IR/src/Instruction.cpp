...
bool Instruction::isStaticCalculable() { return false; }
...
void BinaryInst::assertValid() {
  ...
  assert(
      static_cast<IntegerType *>(get_operand(0)->get_type())->get_num_bits() ==
...
BinaryInst *BinaryInst::create_mod(Value *v1, Value *v2, BasicBlock *bb,
                                   Module *m) {
...
std::string CallInst::print() {
  ...
  instr_ir += print_as_op(this->get_operand(0), false);
  instr_ir += "(";
  for (int i = 1; i < static_cast<int>(this->get_num_operand()); i++) {
    if (i > 1)
      instr_ir += ", ";
...
std::string PhiInst::print() {
  ...
  instr_ir += this->get_operand(0)->get_type()->print();
  instr_ir += " ";
  for (int i = 0; i < static_cast<int>(this->get_num_operand() / 2); i++) {
    if (i > 0)
      instr_ir += ", ";
    instr_ir += "[ ";
...
std::list<std::pair<Value *, BasicBlock *>> PhiInst::getValueBBPair() {
  ...
  for (int i = 0; i < (int)get_num_operand(); i += 2) {
    ret.emplace_back(get_operand(i),
                     dynamic_cast<BasicBlock *>(get_operand(i + 1)));
  }
  return ret;
}

BranchInst::BranchInst(int op_num, BasicBlock *bb)
    : Instruction(Type::get_void_type(bb->get_module()), Instruction::br,
                  op_num, bb) {}
...
ReturnInst::ReturnInst(BasicBlock *bb, size_t num_op)
    : Instruction(Type::get_void_type(bb->get_module()), Instruction::ret,
                  num_op, bb){};
StoreInst::StoreInst(BasicBlock *bb)
    : Instruction(Type::get_void_type(bb->get_module()), Instruction::store, 2,
                  bb){};