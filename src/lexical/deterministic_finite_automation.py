...
def fa_2_dfa(fa: FA) -> DFA:  # NFA确定化
    ...
    for letter in fa.letters:         
      ListOfNewStateList.append(epsilon_closure_Move(S0.epsilonTransitions,letter,fa)) #开始状态的Epsilon闭包通过letter达到的状态组成一个新状态集，存进ListOfNewStateList
      state2 = State(false)                                                                    
    ...
    for StateList in ListOfNewStateList:   #遍历ListOfNewStateList中的状态集，经每个letter通过epsilon_closure_Move得到新的状态集再加进ListOfNewStateList再遍历，重复这个过程直到不产生新的状态集为止
      for letter in fa.letters:
        ...
        if not (ListOfNewStateList.__contains__(NewStateList)): 
          ...
          dict1[StateList].transitions[letter]=dict1[NewStateList]    #设置原状态的transitions映射：StateList映射到的原状态——letter——>NewStateList映射到的新状态
    
    rules = Rules(dict1.get(next(iter(dict1))), dict1.get(list(dict1.keys())[-1])) #取字典中第一个值（状态）和最后一个值作为rule的start和end（python3中字典是按序存储的）
